...
import org.grupo1.finanzas.estimating.domain.model.commands.*; // Importar todos los comandos
...
public record BondParameters(
    ...
    public BondParameters(CreateBondValuationCommand command) {
        this(
                new Money(command.faceValue()), new Money(command.issuePrice()), new Money(command.purchasePrice()),
                command.issueDate(), command.maturityDate(), command.totalPeriods(),
